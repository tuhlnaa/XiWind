...
class ModelEvaluator:
	...
	def plot_precision_recall_curve(self):
		precision, recall, _ = precision_recall_curve(self.targets, self.predictions[:, 1])
...
if __name__ == "__main__":
	...
	print(f"Predicted class: {prediction}, Time taken: {time_taken:.6f} seconds")
	#quit()
	# Batch Prediction
	...
	batch_predictions, avg_time, targets, outputs = batch_predictor.predict_batch(model)
	evaluator = ModelEvaluator(targets, outputs)
	report = evaluator.evaluate()
	print(report)
	print(f"Batch predictions shape: {len(batch_predictions)}, Average time per image: {avg_time:.6f} seconds")
