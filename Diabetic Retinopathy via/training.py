...
from MobileNetV2 import DRModel
from dataloader import prepare_data

# Set the random seed equals zero
# Set the random seed to be zero to make the trainining, validation, and testing processes to be reproducible.
# seed = 0
...
def validate(model, device, loader, epoch, logger):
	test_loss, accuracy, correct, targets, outputs = evaluate_performance(model, device, loader)
	...
	logger.log_report_metrics(
		epoch, 
		report, 
		['No Diabetic Retinopathy', 'Diabetic Retinopathy', 'macro avg', 'weighted avg'], 
...
class Logger:
	...
	def plot_confusion_matrix(self, targets, predictions, epoch):
		...
		sns.heatmap(cm, annot=True, fmt="d", cmap="Blues", ax=ax,
					xticklabels=["No Diabetic Retinopathy", "Diabetic Retinopathy"],
...
def main():
	...
	for epoch in range(start_epoch, num_epochs):
		...
		print(f'Epoch {epoch + 1}/{num_epochs}, Loss: {train_loss:.4f}, Learning Rate: {get_last_lr}')
		
		validation_loss, correct, val_accuracy = validate(model, device, test_loader, epoch, logger)
		print(f'Validation Loss: {validation_loss:.4f}, Accuracy: {correct}/{len(test_loader.dataset)} ({val_accuracy:.2f}%)\n')
